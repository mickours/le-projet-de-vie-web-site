...
```nix
{
  inputs.le-projet-de-vie.url = "github:votre-username/votre-repo";
}
```
...
```nix
{ inputs, pkgs, ... }: {
  imports = [ inputs.le-projet-de-vie.nixosModules.default ];
...
| `dataDir` | Répertoire pour les données persistantes (BD, uploads) | `/var/lib/le-projet-de-vie` |
| `nginx.enable` | Activer un reverse proxy Nginx automatique | `false` |
| `nginx.hostName` | Nom de domaine pour le vhost Nginx | `"localhost"` |

## Gestion des données

Le module crée automatiquement un utilisateur `le-projet-de-vie` et un répertoire dans `/var/lib/le-projet-de-vie`.
